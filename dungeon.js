...
function setGridSize() {
    ...
    const availableHeight = window.innerHeight - 200; // account for header/status
    const tileSize = Math.min(
        60,
        Math.floor(availableWidth / maxX),
        Math.floor(availableHeight / maxY)
    );
    mapContainer.style.setProperty('--tile-size', `${tileSize}px`);
    mapContainer.style.gridTemplateColumns = `repeat(${maxX}, var(--tile-size))`;
    mapContainer.style.gridTemplateRows = `repeat(${maxY}, var(--tile-size))`;
}
